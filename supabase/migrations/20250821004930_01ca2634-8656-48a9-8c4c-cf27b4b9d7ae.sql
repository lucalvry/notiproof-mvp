...
-- Note: The source is already 'woocommerce' from the webhook, but let's check for platform in event_data instead
UPDATE events 
SET 
...
WHERE 
  (event_data->>'platform' = 'woocommerce' OR source = 'woocommerce')
...
WHERE 
  id IN (
    ...
    FROM events 
    WHERE event_data->>'platform' = 'woocommerce' OR event_type = 'purchase'
  )
