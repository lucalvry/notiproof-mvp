-- Enhance testimonial_split_view template with styled text column and placeholders
  ...
  <div class="split-container">
    ...
    <div class="split-content">
      ...
      <div class="split-author">
        <p class="split-name">
          {{#template.author_name}}{{template.author_name}}{{/template.author_name}}
          {{^template.author_name}}Anonymous Customer{{/template.author_name}}
        </p>
        <p class="split-title">
          {{#template.author_position}}{{template.author_position}}{{/template.author_position}}
          {{^template.author_position}}Customer{{/template.author_position}}
          {{#template.author_company}} / {{template.author_company}}{{/template.author_company}}
          {{^template.author_company}}{{#template.author_position}} / Happy Customer{{/template.author_position}}{{/template.author_company}}
        </p>
        {{#template.verified}}
...
.noti-split-view {
  ...
  --split-content-bg: hsl(0 0% 100%);
  --split-content-shadow: 0 4px 12px hsl(0 0% 0% / 0.08);
}
...
.noti-split-view .split-content {
  ...
  justify-content: center;
  background: var(--split-content-bg);
  box-shadow: var(--split-content-shadow);
}
