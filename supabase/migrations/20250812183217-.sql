...
CREATE POLICY IF NOT EXISTS "Support can view all events"
ON public.events
FOR SELECT
USING (has_role(auth.uid(), 'support'::app_role));

CREATE POLICY IF NOT EXISTS "Support can view all profiles"
ON public.profiles
FOR SELECT
USING (has_role(auth.uid(), 'support'::app_role));

CREATE POLICY IF NOT EXISTS "Support can view all widgets"
ON public.widgets
FOR SELECT
USING (has_role(auth.uid(), 'support'::app_role));