-- Create integration_types enum first
DO $$ BEGIN
...
ADD COLUMN IF NOT EXISTS integration_type_new integration_type DEFAULT 'manual',
ADD COLUMN IF NOT EXISTS moderation_status_new moderation_status DEFAULT 'approved',
ADD COLUMN IF NOT EXISTS quality_score INTEGER DEFAULT 50;

-- Copy data and drop old columns if they exist
UPDATE public.events SET integration_type_new = 'manual' WHERE integration_type_new IS NULL;
UPDATE public.events SET moderation_status_new = 'approved' WHERE moderation_status_new IS NULL;

-- Drop the old text column and rename new one
ALTER TABLE public.events DROP COLUMN IF EXISTS integration_type;
ALTER TABLE public.events RENAME COLUMN integration_type_new TO integration_type;

ALTER TABLE public.events DROP COLUMN IF EXISTS moderation_status;
ALTER TABLE public.events RENAME COLUMN moderation_status_new TO moderation_status;
