...
CREATE OR REPLACE FUNCTION public.submit_testimonial_request(
  ...
  _author_role text DEFAULT NULL::text,
  _author_company text DEFAULT NULL::text,
  _author_photo_url text DEFAULT NULL::text,
  _author_website_url text DEFAULT NULL::text,
  _outcome_claim text DEFAULT NULL::text,
  _highlight_phrase text DEFAULT NULL::text
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.proof_objects (
    ...
    NULLIF(trim(_outcome_claim), ''),
    NULLIF(trim(_highlight_phrase), '')
