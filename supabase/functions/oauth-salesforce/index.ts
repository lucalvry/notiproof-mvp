...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const config = await getOAuthConfig(supabase, 'salesforce');
    if (!config) {
      return new Response(
        JSON.stringify({ error: 'Salesforce OAuth not configured by administrators' }),
    ...
    if (action === 'start') {
      ...
      await logOAuthAction(supabase, 'salesforce', 'oauth_start', 'pending', user_id, { state, website_id });
    ...
    if (action === 'callback') {
      ...
      await logOAuthAction(supabase, 'salesforce', 'oauth_complete', 'success', user_id, {});
