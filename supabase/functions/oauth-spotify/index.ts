...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const config = await getOAuthConfig(supabase, 'spotify');
    if (!config) {
      return new Response(
        JSON.stringify({ error: 'Spotify OAuth not configured by administrators' }),
    ...
    if (action === 'start') {
      ...
      await logOAuthAction(supabase, 'spotify', 'oauth_start', 'pending', user_id, { state, website_id });
    ...
    if (action === 'callback') {
      ...
      await logOAuthAction(supabase, 'spotify', 'oauth_complete', 'success', user_id, {});
