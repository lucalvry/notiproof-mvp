...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const config = await getOAuthConfig(supabase, 'twitter');
    if (!config) {
      return new Response(
        JSON.stringify({ error: 'Twitter OAuth not configured by administrators' }),
    ...
    if (action === 'start') {
      ...
      await logOAuthAction(supabase, 'twitter', 'oauth_start', 'pending', user_id, { state, website_id });
    ...
    if (action === 'callback') {
      ...
      await logOAuthAction(supabase, 'twitter', 'oauth_complete', 'success', user_id, {});
