...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const config = await getOAuthConfig(supabase, 'intercom');
    if (!config) {
      return new Response(
        JSON.stringify({ error: 'Intercom OAuth not configured by administrators' }),
    ...
    if (action === 'start') {
      ...
      await logOAuthAction(supabase, 'intercom', 'oauth_start', 'pending', user_id, { state, website_id });
    ...
    if (action === 'callback') {
      ...
      await logOAuthAction(supabase, 'intercom', 'oauth_complete', 'success', user_id, {});
