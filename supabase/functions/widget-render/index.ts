...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: proofs, error: pErr } = await supabase
      .from("proof_objects")
      .select(
        "id, type, content, highlight_phrase, author_name, author_avatar_url, author_photo_url, author_role, author_company, author_company_logo_url, author_website_url, cta_label, cta_url, rating, media_url, poster_url, product_image_url, product_url, source, created_at",
      )
      .eq("business_id", businessId)
      .eq("status", "approved")
